...
def send_order(direction, lot, atr, candle_time):
    ...
    if result.retcode == mt5.TRADE_RETCODE_DONE:
        ...
        print(f"✅ TRADE PLACED | {direction} | lot={lot} | risk=$50")
    else:
...
while True:
    ...
    if direction == "BUY" and pred4 <= last4:
        print(f"⛔ BLOCKED BY 4H | BUY | pred4={pred4:.2f} <= last4={last4:.2f}")
        time.sleep(60)
        continue

    if direction == "SELL" and pred4 >= last4:
        print(f"⛔ BLOCKED BY 4H | SELL | pred4={pred4:.2f} >= last4={last4:.2f}")
        time.sleep(60)
        continue
